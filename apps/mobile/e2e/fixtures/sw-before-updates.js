...
const CACHE = 'compass-shell-__VERSION__';
...
self.addEventListener('activate', event => {
  event.waitUntil((async () => {
    for (const name of await caches.keys()) if (name.startsWith('compass-shell-') && name !== CACHE) await caches.delete(name);
...
self.addEventListener('fetch', event => {
  ...
  event.respondWith((async () => {
    const cached = await caches.match(notificationEntry ? self.registration.scope : event.request);
